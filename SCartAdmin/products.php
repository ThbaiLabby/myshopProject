    ...
    <div id="content">
        <?PHP
            if(isset($_SESSION['adminlogin'])){
                ...
                $tblProducts = new ajaxCRUD("Products", "products", "id");

                //don't show the primary key in the table
                //$tblCustomer->omitPrimaryKey();

                // display headers as reasonable titles
                $tblProducts->displayAs("Make", "Make");
                $tblProducts->displayAs("Model", "Model");
                $tblProducts->displayAs("Description", "Description");
                $tblProducts->displayAs("Category", "Category");
                $tblProducts->displayAs("Special", "Special");
                $tblProducts->displayAs("Image", "Image");

                // add the filter box (above the table)
                $tblProducts->addAjaxFilterBox("Make");

                // actually show to the table
                $tblProducts->showTable();
            }  else {
                //display login form
                ?>
                <form method="POST">
