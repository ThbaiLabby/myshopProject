    ...
    <div id="content">
        <?PHP
            if(isset($_SESSION['adminlogin'])){
                ...
                $tblOrders = new ajaxCRUD("Orders", "oders", "id");
                //don't show the primary key in the table
                //$tblCustomer->omitPrimaryKey();

                // display headers as reasonable titles
                $tblOrders->displayAs("id", "Order Number");
                $tblOrders->displayAs("user", "User ID");
                $tblOrders->displayAs("processed", "Processed");

                // add the filter box (above the table)
                $tblOrders->addAjaxFilterBox("user");
